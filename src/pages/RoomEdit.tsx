...
import { RoomData, RoomInfo } from '../types/room';
...
const RoomEdit: React.FC = () => {
  ...
  const { roomId, roomName, roomDescription, thumbnail } = useRoomStore()
  ...
  useEffect(() => {
    const fetchImageFile = async () => {
      ...
      try {
        const imgFile = await createImageFile(
          thumbnail,
          `${roomName}-thumbnail`
        );
        setRoomData((prev) => ({
        ...
        setError('Failed to load the thumbnail image');
      }
    };
